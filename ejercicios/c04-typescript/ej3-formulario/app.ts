const catalogoListado = document.getElementById("listado") as HTMLUListElement;
...
const buttonFiltrar = document.getElementById("filtrar") as HTMLButtonElement;
const buttonMostrarDisponibles = document.getElementById("mostrarDisponibles") as HTMLButtonElement;
const buttonMostrarTodos = document.getElementById("mostrarTodos") as HTMLButtonElement;

const libroFormulario = document.getElementById("libroFormulario") as HTMLFormElement;
const errorFormulario = document.getElementById("errorFormulario") as HTMLDivElement;
...
libroFormulario.addEventListener("submit", (event: SubmitEvent) => {
  ...
  catalogo.push(validado as Libro);
  renderizar(catalogo);
  libroFormulario.reset();
})

function validateForm(libro: Libro): Libro | boolean {
  const titulo = libro.titulo.trim();
  const autor = libro.autor.trim();
  if (titulo === "" || autor === "") {
    return false;
  ...
  let generoOpcional: { genero?: string } = {};
